...
__ALIGN_BEGIN uint8_t USBD_CDC_CfgHSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END
		...
		0x00, 0x02 * 2, /* bNumInterfaces: 2 interface */
		...
		//Add 1 IAD class here // this one is for COM port 1
				...
				CDC_HS_BINTERVAL, /* bInterval: x10 */
				/*---------------------------------------------------------------------------*/
				...
				0x03, /* bmAttributes: Interrupt */
				LOBYTE(CDC_CMD_PACKET_SIZE), /* wMaxPacketSize: */
				HIBYTE(CDC_CMD_PACKET_SIZE), 0x10, /* bInterval: 0xff? */
				/*---------------------------------------------------------------------------*/
...
__ALIGN_BEGIN uint8_t USBD_CDC_CfgFSDesc[USB_CDC_CONFIG_DESC_SIZ] __ALIGN_END
		...
		USB_DESC_TYPE_CONFIGURATION, /* bDescriptorType: Configuration */
		USB_CDC_CONFIG_DESC_SIZ, /* wTotalLength:no of returned bytes */
		0x00, 0x04, /* bNumInterfaces: 4 interface */
		0x01, /* bConfigurationValue: Configuration value */
		0x00, /* iConfiguration: Index of string descriptor describing the configuration */
		0xC0, /* bmAttributes: self powered */
		0x32, /* MaxPower 0 mA */
		...
		//Add 1 IAD class here // this one is for COM port 1
				...
				LOBYTE(CDC_CMD_PACKET_SIZE), /* wMaxPacketSize: */
				HIBYTE(CDC_CMD_PACKET_SIZE),
				CDC_FS_BINTERVAL, /* bInterval: 0xff? */
				/*---------------------------------------------------------------------------*/
...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	if (pdev->dev_speed == USBD_SPEED_HIGH) {
		...
		pdev->ep_out[CDC_OUT_EP2 & 0xFU].is_used = 1U;	//RGE add
		...
		pdev->ep_in[(CDC_IN_EP2) & 0xFU].is_used = 1U;	//RGE add
	...
	USBD_EP_TYPE_INTR,
	CDC_CMD_PACKET_SIZE);
	pdev->ep_in[CDC_CMD_EP2 & 0xFU].is_used = 1U;	//RGE add
	...
	if (pdev->pClassData == NULL) {
		...
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			...
			CDC_OUT_EP2, hcdc->RxBuffer,
			CDC_DATA_HS_OUT_PACKET_SIZE);
		} else {
			/* Prepare Out endpoint to receive next packet */
			USBD_LL_PrepareReceive(pdev,
			CDC_OUT_EP1, hcdc->RxBuffer,
			CDC_DATA_FS_OUT_PACKET_SIZE);
			/* Prepare Out endpoint to receive next packet */
			USBD_LL_PrepareReceive(pdev,
			CDC_OUT_EP2, hcdc->RxBuffer,
...
static uint8_t USBD_CDC_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
	...
	/* Close EP IN */
	USBD_LL_CloseEP(pdev, CDC_IN_EP1);
	pdev->ep_in[CDC_IN_EP1 & 0xFU].is_used = 0U;

	/* Close EP OUT */
	USBD_LL_CloseEP(pdev, CDC_OUT_EP1);
	pdev->ep_out[CDC_OUT_EP1 & 0xFU].is_used = 0U;

	/* Open Command IN EP */
	USBD_LL_CloseEP(pdev, CDC_CMD_EP1);
		...
		if (req->wLength) {
			if (req->bmRequest & 0x80U) {
				((USBD_CDC_ItfTypeDef*) pdev->pUserData)->Control(req->bRequest,
						(uint8_t*) hcdc->data, req->wLength, req->wIndex);
...
static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	...
	if (pdev->pClassData != NULL) {
		((USBD_CDC_ItfTypeDef*) pdev->pUserData)->Receive(hcdc->RxBuffer, &hcdc->RxLength, epnum);	//RGE add ep
...
static uint8_t USBD_CDC_EP0_RxReady(USBD_HandleTypeDef *pdev) {
	...
	if ((pdev->pUserData != NULL) && (hcdc->CmdOpCode != 0xFF)) {
		((USBD_CDC_ItfTypeDef*) pdev->pUserData)->Control(hcdc->CmdOpCode,
				(uint8_t*) hcdc->data, hcdc->CmdLength, pdev->request.wIndex);
		hcdc->CmdOpCode = 0xFF;
 ...
 * @retval status
 */
uint8_t USBD_CDC_TransmitPacket(USBD_HandleTypeDef *pdev, uint16_t ep_in) {
	USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;
	...
	if (pdev->pClassData != NULL) {
		if (hcdc->TxState == 0U) {
			...
			/* Update the packet total length */
			pdev->ep_in[ep_in & 0xFU].total_length = hcdc->TxLength;	//RGE add ep_out

			/* Transmit next packet */
			USBD_LL_Transmit(pdev, ep_in, hcdc->TxBuffer, hcdc->TxLength);
 ...
 * @retval status
 */
uint8_t USBD_CDC_ReceivePacket(USBD_HandleTypeDef *pdev, uint16_t ep_out) {
	USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*) pdev->pClassData;

	/* Suspend or Resume USB Out process */
	if (pdev->pClassData != NULL) {
		if (pdev->dev_speed == USBD_SPEED_HIGH) {
			/* Prepare Out endpoint to receive next packet */  //RGE add ep
			USBD_LL_PrepareReceive(pdev, ep_out, hcdc->RxBuffer,
			CDC_DATA_HS_OUT_PACKET_SIZE);
